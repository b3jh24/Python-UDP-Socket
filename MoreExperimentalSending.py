...
port = 5005
addr = (host,port)
s.setsockopt(socket.SOL_SOCKET,socket.SO_SNDBUF, 30485760)
...
packetQueue = [None] * 8000        #Empty list that contains each packet sent - used for retransmissions (can only contain the
...
def run():
    ...
    numSent = 0
    loop = 0
    howManyToSend = 8000      #Number of chunks the client wants us to send - hardcoded for now
    ...
    '''
    if(conn.recv(tcp_buf) == "Send me some chunks"):
        logging.info("He wants us to send him 60 chunks of data, let's do it!")
        conn.send("Chunks on their way brah!")
    else:
    ...
    while(totalNumSent < getNumChunksToSend() and keepGoing):
        while(numSent < howManyToSend and keepGoing):
            if(totalNumSent > getNumChunksToSend() - howManyToSend):
            ...
            for chunk in read_in_chunks(f):               
                ...
                if(s.sendto(chunk,addr) and s.sendto(str(local_ID),addr)):
                    packetQueue[local_ID - (loop * howManyToSend)] = chunk
                    numSent += 1
                    local_ID += 1
                    totalNumSent += 1
    end = time.time()
