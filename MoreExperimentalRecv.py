...
sock.setsockopt(socket.SOL_SOCKET,socket.SO_RCVBUF, 30485760)
...
numChunksRequested = 8000  # The number of chunks we're gonna ask for
...
'''
tcp_sock.send("Send me some chunks");
...
def write_to_file(path, buffer, terminate_signal):
    ...
    Args:
        ...
        buffer - the list (if you will) containing the data to be written
...
def RecvResentPackets(f, numBeingResent):
...
try:
    while(canRecv):
        ...
        if(packID.isdigit() and dataChunk):
            numRecv += 1
            totalNumRecv += 1
            #print "TotalNumRecv: " , totalNumRecv
            '''
            Once we've received an ID, we want to store it in a list. We put it in the index that corresponds to it's ID number
            and flag it as received (with a 1). Once we've timed out, we can check this list to see what we're missing
            '''
            chunkIDsRecv[int(packID) - (loop*numChunksRequested)] = 1
            buffer.appendleft(dataChunk)  # append to LEFT of buffer
            ...
            if(totalNumRecv*buf >= progressMultiplier * .05 * totalNumBytesToRecv):
                print str(progressMultiplier*5) + "% of data received"
    ...
    canRecv = True
    if(RecvResentPackets(f, len(packetsINeed))):
        ...
        #print "What the fuck do I do now??"
        print "FIXME"
